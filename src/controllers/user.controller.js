...
const loginUser = asyncHandler(async(req,res)=>{
    ...
    //now we need to send data in cookies
    const options={
        httpOnly: true,
        secure: true
    }
...
const changeCurrentPassword=asyncHandler(async(req,res)=>{
    ...
    const isPasswordCorrect=await user.isPaasswordCorrect(oldPassword)

    if(!isPasswordCorrect){
        throw new ApiError(400,"Invalid passwprd entered")
    }
...
const getWatchHistory=asyncHandler(async(req,res)=>{
    const user=await URLSearchParams.aggregate([
        {
            $match:{
